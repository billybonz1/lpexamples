...
editableItems["[class^='icon-']"] = ['color', 'font-size'];
editableItems["[class*=' icon-']"] = ['color', 'font-size'];
editableItems['.bg.bg1'] = ['background-color'];
editableItems['nav a:not(.dropdown-toggle), a.edit'] = ['color', 'font-weight', 'text-transform'];
...
editableItems['.editContent'] = ['color', 'font-size', 'font-family'];
...
editableItems['.bg', '.bg.bg1, .bg.bg2, .header10, .header11, .topnav'] = ['background-image', 'background-color'];
...
var editableContent = ['p', 'h1', 'h2', 'h3', 'h4', '.editContent', '.navbar a', 'button', 'a.btn', '.footer a:not(.fa)', '.tableWrapper'];
...
$( window ).resize(function() {
  ...
  if(windowHeight < 700) {
    $('.make-logo').css('position', 'relative').css('margin-left', 45).css('right', 0);
  }
  else {
    $('.make-logo').attr('style', '');
  }
});
	...
	$('#previewModal').on('shown.bs.modal', function (e) {
		...
		$('#pageList > ul:visible').each(function(){
			...
			$(this).find('iframe').each(function(){
				...
				var attr = $(this).attr('data-sandbox');
				if (typeof attr !== typeof undefined && attr !== false) {
					theContents = $('#sandboxes #'+attr).contents().find( pageContainer ).clone();
				} else {
					theContents = $(this).contents().find( pageContainer ).clone();
				}
				//remove .frameCovers
	...
	$('#exportModal').on('shown.bs.modal', function (e) {
		...
		$('#pageList > ul').each(function(){
			...
			$(this).find('iframe').each(function(){
				...
				theContents = theContents.clone();
				//remove .frameCovers
				theContents.find('.frameCover').each(function(){
					$(this).remove();
				});
				theContents.find('.visible').each(function() {
            $(this).removeClass('visible');
        });
        theContents.find('.fadeIn').each(function() {
            $(this).removeClass('fadeIn');
        });
        theContents.find('.show-svg').each(function() {
            $(this).removeClass('show-svg');
        });
        theContents.find('.hide-svg').each(function() {
            $(this).removeClass('hide-svg');
        });
        theContents.find('img').each(function() {
            var newsrc = $(this).attr('src').replace('../../common-files/', '');
            $(this).attr('src', newsrc);
        });
        theContents.find('.map').each(function() {
            $(this).attr('style', '');
            $(this).empty();
            mapScripts = '<script src="http://maps.google.com/maps/api/js?sensor=true"></script><script src="plugins/google-maps/gmaps.min.js"></script><script src="https://google-maps-utility-library-v3.googlecode.com/svn-history/r391/trunk/markerwithlabel/src/markerwithlabel.js"></script><script src="js/map.js"></script>';
        });

        theContents.find('.owl-carousel').each(function() {
            $(this).attr('style', '');
            $(this).removeClass('owl-theme');
			...
			var themeHeader = '<!DOCTYPE HTML>\n' +
                        ...
                        '      <link rel="stylesheet" href="plugins/magnific/magnific-popup.min.css" rel="stylesheet">\n' +
                        '      <link rel="stylesheet" href="plugins/rateit/rateit.css"rel="stylesheet">\n' +
                        '      <!-- END PAGE STYLE -->\n' +
                        '      <!-- [if lt IE 9]>\n' +
                        ...
                        '      </div>\n' +
                        '      <!-- END PRELOADER -->';
      var themeFooter = '  <a href="#" class="scrollup"><i class="fa fa-angle-up"></i></a>\n' +
			...
			newInput.val( themeHeader +$('iframe#skeleton').contents().find('html').html()+ themeFooter + mapScripts )
		})
	});
